...
func SnapshotAPIParallel(dir string) map[string]APIPackage {
	pkgPaths := listPackages(dir)
	...
	outCh := make(chan result, len(pkgPaths))
	// TODO: --workers CLI param
	sem := make(chan struct{}, 8)

	for _, pkgPath := range pkgPaths {
		pkgPath := pkgPath // capture
		sem <- struct{}{}
		go func() {
			defer func() { <-sem }()
			p, api := SnapshotOnePackage(dir, pkgPath)
			outCh <- result{p, api}
		}()
	}
	...
	for i := 0; i < len(pkgPaths); i++ {
		res := <-outCh
		api[res.pkgPath] = res.api
	}

	return api
}

func SnapshotOnePackage(dir, pkgPath string) (string, APIPackage) {
	cfg := &packages.Config{
		Mode: packages.NeedName | packages.NeedTypes | packages.NeedImports,
		Dir:  dir,
	}
	...
	if len(pkgs) != 1 {
		log.Fatalf("unexpected number of packages for %s: got %d", pkgPath, len(pkgs))
	}
	pkg := pkgs[0]
